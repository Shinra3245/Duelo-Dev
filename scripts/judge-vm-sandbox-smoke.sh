...
SMOKE_SUFFIX="${JUDGE_VM_SANDBOX_SUFFIX:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
if [[ ! "$SMOKE_SUFFIX" =~ ^[A-Za-z0-9][A-Za-z0-9-]{0,47}$ ]]; then
  printf 'Error: JUDGE_VM_SANDBOX_SUFFIX debe ser alfanumérico y contener hasta 48 caracteres.\n' >&2
  exit 2
fi
readonly REMOTE_DIR="/home/judge/duelodev-sandbox-smoke-${SMOKE_SUFFIX}"
